...
import com.gateway.controller.AuthenticationController;
...
@Service
public class CredentialDetailsServiceImpl implements UserDetailsService, CredentialDetailsService {
...
//			loginUser = credentialDao.findByUsernameAndChannelNameAndIsEnable(authenticationRequest.getUsername(),authenticationRequest.getChannel(),true);
			loginUser = credentialDao.findByUsernameAndChannelName(authenticationRequest.getUsername(),authenticationRequest.getChannel());
			if(loginUser!=null) {
			try {				
				...
				response = new AuthenticationResponse(Constants.ResponseCodes.DISABLED_EXCEPTION, Constants.ResponseDescription.DISABLED_EXCEPTION);
				return response;
			} catch (BadCredentialsException e) {
				LOG.info("CredentialDetailsServiceImpl - INVALID CREDENTIALS");
				response = new AuthenticationResponse(Constants.ResponseCodes.UNAUTHORISED, Constants.ResponseDescription.UNAUTHORISED_WRONG_CREDENTIALS);
				return response;
			...
			response = new AuthenticationResponse(Constants.ResponseCodes.OK,Constants.ResponseDescription.OK, expiry, token);
			
		}else {
			response = new AuthenticationResponse(Constants.ResponseCodes.UNAUTHORISED,Constants.ResponseDescription.UNAUTHORISED_WRONG_CHANNEL);
			return response;
			}
		} catch (Exception e) {
			response = new AuthenticationResponse(Constants.ResponseCodes.UNABLE_TO_PROCESS,ResponseDescription.UNABLE_TO_PROCESS);
			...
			try {
				...
				auditLoggingService.auditLog("Authentication", response.getResponseCode(),response.getResponseDescription(), requestAsString,
						responseAsString, requestDatetime, requestDatetime,
						null,null,null,authenticationRequest.getChannel(),authenticationRequest.getUsername());
	...
	public UserDetails loadUserByUsername(String username) throws UsernameNotFoundException {
		LOG.info("CredentialDetailsServiceImpl - Finding user in credentials Table - username:", username);
		try {
		Credential user =  credentialDao.findByUsername(username);
		if (user == null) {
			LOG.info("CredentialDetailsServiceImpl - User not found with username:", username);
			throw new UsernameNotFoundException("User not found with username: " + username);
		}
		LOG.info("CredentialDetailsServiceImpl - User found with username:", username);
