...
import com.gateway.model.mpay.response.billinquiry.vms.VmsGetvoucher;
import com.gateway.model.mpay.response.billinquiry.vms.VmsOfflineGetVoucherResponse;
...
public class BillInquiryServiceImpl implements BillInquiryService {
	...
	private String bppraTenderChallanInquiryCall;
								...
								if (billInquiryValidationResponse != null && billInquiryValidationResponse
									...
									////////// PU ///////
										...
										case BillerConstant.PU.PU:
											...
											billInquiryResponse = billInquiryPU(request, httpRequestData);
											break;
...
//			}
		...
		Exception ex) {
			...
			try {
				...
				paymentLoggingService.paymentLog(responseDate, responseDate, rrn, stan,
						...
						bankName, bankCode, branchName, branchCode, username, "", dueDate, billingMonth);
				...
				if(getVoucherResponse.getResponse().getOfflineBillerGetvoucher().getGetvoucher().getBillStatus().equalsIgnoreCase("EXPIRED")) {
					...
					if (getVoucherResponse.getResponse().getOfflineBillerGetvoucher() != null) {
						...
						if (getVoucherResponse.getResponse().getOfflineBillerGetvoucher().getGetvoucher()
							...
							if (combinedPaymentLogView != null) {
	...
	public BillInquiryResponse billInquiryVms(HttpServletRequest httpRequestData, BillInquiryRequest request,
		...
		BillInquiryResponse response = null;
		VmsOfflineGetVoucherResponse getVoucherResponse = null;
		VmsGetvoucher getvoucher = null;
		Info info = null;
		Date strDate = new Date();
		String rrn = request.getInfo().getRrn(); // utilMethods.getRRN();
		String stan = request.getInfo().getStan(); // utilMethods.getStan();
		...
		try {
			...
			ArrayList<String> inquiryParams = new ArrayList<String>();
			inquiryParams.add(Constants.MPAY_REQUEST_METHODS.VMS_OFFLINE_BILLER_INQUIRY.trim());
			inquiryParams.add(parentBiller.trim());
			inquiryParams.add(subBiller.trim());
			inquiryParams.add(request.getTxnInfo().getBillNumber().trim());
			inquiryParams.add(channel);// channel
			...
			getVoucherResponse = serviceCaller.get(inquiryParams, VmsOfflineGetVoucherResponse.class, rrn,
					Constants.ACTIVITY.BillInquiry, "");
		...
		try {
			...
			try {
				...
				paymentLoggingService.paymentLog(responseDate, responseDate, rrn, stan,
						...
						transactionStatus, address, dbTotal, channel, dbBillStatus, request.getTxnInfo().getTranDate(),
						request.getTxnInfo().getTranTime(), province, transAuthId, bankName, bankCode, branchName,
						branchCode, username, "", dueDate, billingMonth);
		...
		try {
			...
			try {
				...
				paymentLoggingService.paymentLog(requestedDate, new Date(), rrn, stan,
						...
						(billerNameRes == null || billerNameRes.equals("") ? billerName : billerNameRes),
						request.getTxnInfo().getBillNumber(), request.getTxnInfo().getBillerId(),
						(amountDueDateRes == null || amountDueDateRes.equals("") ? amountInDueToDate
								: amountDueDateRes),
						(amounAfterDateRes == null || amounAfterDateRes.equals("") ? amountAfterDate
								: amounAfterDateRes),
						...
						(billingMonthRes == null || billingMonthRes.equals("") ? billingMonth : billingMonthRes), "",
		...
		try {
			...
			try {
				...
				PaymentLog savedPaymentLog = paymentLoggingService.paymentLog(responseDate, responseDate, rrn, stan,
						...
						transactionStatus, address, dbTotal, channel, billStatus, request.getTxnInfo().getTranDate(),
						request.getTxnInfo().getTranTime(), province, transAuthId, bankName, bankCode, branchName,
						branchCode, username, feeDetail, dueDate, billingMonth);
				...
				/// TenderAuth Success
					...
					if (tenderTokenInquiry.getStatus() == 200) {
						...
						HttpResponse<String> tenderChallanInquiry = utilMethods.tenderChallanInquiryRequest(
								bppraTenderChallanInquiryCall, request.getTxnInfo().getBillNumber(),
				...
				/// SupplierAuth Success
					...
					if (supplierTokenInquiry.getStatus() == 200) {
						...
						if (supplierChallanInquiry.getStatus() == 200) {
							...
							LOG.info("SupplierChallanInquiry : Success");
		...
		try {
			...
			if (biseKohatBillInquiryResponse != null) {
				...
				///// Inquiry Success Response
					...
					billingMonth = utilmethod.formatDateFormat(biseKohatBillInquiryResponse.getBiseKohatResponse()
	...
	public BillInquiryResponse billInquiryPU(BillInquiryRequest request, HttpServletRequest httpRequestData) {
		...
		LOG.info("PU Bill Inquiry Request {} ", request.toString());
